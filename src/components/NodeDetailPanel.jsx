...
export default function NodeDetailPanel({ node, onClose, onDelete, onStartConnect }) {
  if (!node) return null
      ...
      <div className="detail-panel-content">
        <div className="detail-panel-header">
          {node.imageUrl ? (
            ...
            <div className="detail-panel-image" style={{
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              background: 'var(--bg-surface-hover)', fontSize: '36px',
            }}>
              {emoji}
            </div>
          )}
          ...
          <button className="modal-close" onClick={onClose} id="detail-close">✕</button>
        </div>
        ...
        <div className="detail-panel-actions">
          <button className="btn btn-secondary" onClick={() => { onStartConnect?.(node.id); onClose?.() }} id="detail-connect">
            🔗 Connect
          </button>
          <button className="btn btn-danger" onClick={() => { onDelete?.(node.id); onClose?.() }} id="detail-delete">
            🗑 Delete
          </button>
        </div>
      </div>
